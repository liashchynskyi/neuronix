            ...
            new Thread(() -> {
                try {
                    ...
                    BalancedPathFilter       pathFilter = new BalancedPathFilter(rng,
                                                                                 ...
                                                                                 labelMaker);
                    Utils.updateTrainLabel(this.label,
                                           "Формування навчальної та тестової вибірки...");
                    InputSplit[] filesInDirSplit = filesInDir.sample(pathFilter, (int) splitTrainTest.getValue(),
                                                                     (int) 100 -
                                                                     this.splitTrainTest.getValue());
                    ...
                    ImageRecordReader recordReader = new ImageRecordReader(encoded.getImageSize(),
                                                                           ...
                                                                           encoded.getChannels(),
                                                                           labelMaker);
                    ImageTransform    transform    = new MultiImageTransform(rng, new CropImageTransform(
                    ...
                    Utils.updateScores(a, p, r, f, new double[]{ eval.accuracy(), eval.precision(),
                                                                 eval.recall(), eval.f1() });
                    if (prefs.currentSaveStateProperty()
                             .get()) {
                        log.info("Save model....");
